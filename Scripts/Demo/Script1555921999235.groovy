...
if (WebUI.getNumberOfTotalOption(findTestObject('Input/select_Facility')) == 3) {
    //比對選項內容是否正確
    WebUI.verifyOptionsPresent(findTestObject('Input/select_Facility'), ['Tokyo CURA Healthcare Center', 'Hongkong CURA Healthcare Center'
            , 'Seoul CURA Healthcare Center'])

    //選擇項目
    WebUI.selectOptionByValue(findTestObject('Input/select_Facility'), findTestData('demotest').getValue('Facility', 1), 
        false)

    //驗證是否選擇正確項目
    WebUI.verifyOptionSelectedByValue(findTestObject('Input/select_Facility'), findTestData('demotest').getValue('Facility', 
            1), false, 10)
}
...
//輸入日期
WebUI.setText(findTestObject('Input/input__VisitDate'), findTestData('demotest').getValue('Date', 1))
...
//輸入Comment資料
WebUI.setText(findTestObject('Input/input__textComment'), findTestData('demotest').getValue('Comment', 1))
...
//比對Facility欄位是否與輸入相同
WebUI.verifyElementText(findTestObject('Verify/verify__Facility'), findTestData('demotest').getValue('Facility', 1))

//Data ApplyForHospital欄位若為Y，比對Apply For Hospital Readmission是否為YES，否則NO
if (findTestData('demotest').getValue('ApplyForHospital', 1) == 'Yes') {
    WebUI.verifyElementText(findTestObject('Verify/verify__ApplyForHospitalReadmission'), 'Yes')
} else {
    WebUI.verifyElementText(findTestObject('Verify/verify__ApplyForHospitalReadmission'), 'No')
}

//比對Healthcare Program欄位是否與輸入相同
WebUI.verifyElementText(findTestObject('Verify/verify__HealthcareProgram'), findTestData('demotest').getValue('HealthcareProgram', 
        1))

//比對date欄位是否與輸入相同
WebUI.verifyElementText(findTestObject('Verify/verify__date'), findTestData('demotest').getValue('Date', 1))

//比對comment是否與輸入相同
WebUI.verifyElementText(findTestObject('Verify/verify__comment'), findTestData('demotest').getValue('Comment', 1))
